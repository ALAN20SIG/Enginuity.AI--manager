      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-xs font-mono text-muted-foreground">{sprint.daysRemaining} days remaining</div>
          </div>
          <div className="h-64">
            <ResponsiveContainer>
        ...
        <Card className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground font-mono mt-1">{sprint.completedPoints} / {sprint.totalPoints} pts</div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {(["todo", "progress", "review", "done"] as const).map((col) => (
          <Card key={col} className="p-4 space-y-2">
            <div className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-2">{col}</div>
            {sprint.columns[col].map((t) => (
              <div key={t.id} className="p-3 rounded border border-border bg-secondary/30 text-xs space-y-1">
                <div className="font-mono text-[10px] text-muted-foreground">{t.id} · {t.points}pt</div>
                <div className="text-foreground">{t.title}</div>
                <div className="font-mono text-[10px] text-muted-foreground">{t.assignee} · {t.priority}</div>
              </div>
            ))}
          </Card>
        ))}
