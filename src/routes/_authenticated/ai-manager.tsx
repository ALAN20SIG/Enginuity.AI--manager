    ...
    <div className="flex h-full">
      <aside className="w-60 border-r border-border p-4 hidden lg:flex flex-col">
        ...
        <div className="mt-6 space-y-1 text-sm">
          ...
          {["Sprint Alpha Phoenix review", "Weekly exec report", "PR #833 security audit"].map(
            (t) => (
              <button
                key={t}
                className="block w-full text-left px-3 py-2 rounded hover:bg-white/[0.03] text-muted-foreground hover:text-foreground truncate"
              >
                {t}
              </button>
            ),
        ...
        <div className="px-6 py-4 border-b border-border flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">
              MCP context · 7 servers · 128k tokens
            </div>
          </div>
        </div>
          ...
          {messages.length === 0 ? (
            <div className="max-w-2xl mx-auto">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Try one of these or ask anything about your engineering org.
              </p>
            ...
            <div className="max-w-3xl mx-auto space-y-6">
              {messages.map((m) => (
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-lg px-4 py-3 text-sm leading-relaxed ${m.role === "user" ? "bg-primary text-primary-foreground" : "bg-card border border-border"}`}
                  >
                    {m.parts.map((p, i) =>
                      p.type === "text" ? (
